...
- **Plugin-free outgoing email** — `wp_mail()` relays through msmtp, configured
  entirely from `SMTP_*` env vars (works in web + cron). See [MAIL.md](MAIL.md).
...
# 4) Preflight: catch placeholder secrets, bad values, missing proxy network
./verify.sh

# 5) Build + start (first build takes a few minutes)
...
| `TRUSTED_PROXY_CIDRS` | private ranges | Networks Apache (`mod_remoteip`) trusts for `X-Forwarded-For`. Narrow to your edge proxy's subnet for best security. |
...
| `HEALTHCHECK_URL` | — | Optional monitor URL (e.g. Healthchecks.io) pinged on success, `<URL>/fail` on failure. Empty = disabled. |
| `BACKUP_MAX_AGE_HOURS` | `26` | Container reports **unhealthy** if no successful backup within this window. |
...
> **Don't let backups fail silently.** The `backup` container runs a
> healthcheck (`docker compose ps` shows it unhealthy if no success within
> `BACKUP_MAX_AGE_HOURS`) and, if `HEALTHCHECK_URL` is set, pings an external
> dead-man's-switch on every run. Wire it to Healthchecks.io / Uptime Kuma so a
> broken backup pages you instead of going unnoticed for weeks.
...
├── verify.sh               # preflight: validate .env before deploy
├── php/php.ini             # PHP tuning (memory, uploads, OPcache+JIT)
├── mariadb/my.cnf          # MariaDB/InnoDB tuning
├── apache/migration.conf   # Apache timeouts / upload limits / trusted proxy
├── mail/wp-sendmail.sh     # plugin-free SMTP shim (msmtp) for outgoing mail
├── backup/                 # S3 backup service (Dockerfile, backup.sh, entrypoint.sh, healthcheck.sh)
├── proxy-caddy/            # optional Caddy edge proxy (when there's no NPM)
├── ci/                     # CI helper scripts (env-var completeness check)
├── .github/workflows/      # CI: lint, compose validate, build + Trivy scan
...
└── MAIL.md                 # plugin-free outgoing email via SMTP
```
...
- **[MAIL.md](MAIL.md)** — plugin-free outgoing email via SMTP (msmtp).
...
- **Почта без плагина** — `wp_mail()` уходит через msmtp, настройка целиком из
  переменных `SMTP_*` (работает в веб-контейнере и в кроне). См. [MAIL.md](MAIL.md).
...
# 4) Preflight: поймать placeholder-секреты, плохие значения, отсутствие сети
./verify.sh

# 5) Собрать + запустить (первая сборка — несколько минут)
docker compose up -d --build
...
| `TRUSTED_PROXY_CIDRS` | приватные сети | Сети, которым Apache (`mod_remoteip`) доверяет `X-Forwarded-For`. Для лучшей безопасности сузьте до подсети вашего edge-прокси. |
...
| `HEALTHCHECK_URL` | — | Необязательный URL монитора (напр. Healthchecks.io): пинг при успехе, `<URL>/fail` при ошибке. Пусто = выключено. |
| `BACKUP_MAX_AGE_HOURS` | `26` | Контейнер становится **unhealthy**, если успешного бэкапа не было в этом окне. |
...
> **Бэкап не должен падать молча.** Контейнер `backup` имеет healthcheck
> (`docker compose ps` покажет unhealthy, если успеха не было в пределах
> `BACKUP_MAX_AGE_HOURS`) и, если задан `HEALTHCHECK_URL`, пингует внешний
> dead-man's-switch на каждом запуске. Подключите Healthchecks.io / Uptime Kuma —
> и сломанный бэкап уведомит вас, а не останется незамеченным неделями.
...
├── verify.sh               # preflight: проверка .env перед деплоем
├── php/php.ini             # тюнинг PHP (память, загрузки, OPcache+JIT)
├── mariadb/my.cnf          # тюнинг MariaDB/InnoDB
├── apache/migration.conf   # таймауты Apache / лимиты загрузки / доверенный прокси
├── mail/wp-sendmail.sh     # SMTP-обёртка (msmtp) для отправки почты без плагина
├── backup/                 # сервис бэкапа в S3 (Dockerfile, backup.sh, entrypoint.sh, healthcheck.sh)
├── proxy-caddy/            # опциональный edge-прокси Caddy (когда нет NPM)
├── ci/                     # вспомогательные скрипты CI (проверка полноты env)
├── .github/workflows/      # CI: линт, валидация compose, сборка + скан Trivy
├── README.md               # ← вы здесь
...
└── MAIL.md                 # отправка почты через SMTP без плагина
```
...
- **[MAIL.md](MAIL.md)** — отправка почты через SMTP без плагина (msmtp).
